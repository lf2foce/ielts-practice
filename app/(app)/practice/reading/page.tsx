...
import { getAllTests, getFullMockTests, getPracticeTests, type ReadingTest } from "@/lib/reading-tests"

export default async function ReadingPracticePage() {
    const fullMockTests = getFullMockTests()
    const practiceTests = getPracticeTests()
    const featuredMock = fullMockTests[0]
        ...
        <div className="flex flex-col p-6 bg-slate-50 dark:bg-[#020617] min-h-full">
                            ...
                            <div className="flex-1 text-center lg:text-left">
                                ...
                                <div className="flex flex-wrap items-center justify-center lg:justify-start gap-6 mb-8 text-white/60">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-sm font-bold uppercase tracking-tight">{featuredMock.durationMinutes} Minutes</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <Layers className="w-4 h-4 text-indigo-400" />
                                        <span className="text-sm font-bold uppercase tracking-tight">{featuredMock.passages.length} Passages</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <FileText className="w-4 h-4 text-indigo-400" />
                                        <span className="text-sm font-bold uppercase tracking-tight">{featuredMock.questions.length} Questions</span>
                                    </div>
                                </div>
                                <Button size="lg" className="h-16 px-10 rounded-2xl bg-white text-slate-900 hover:bg-slate-100 font-black text-lg shadow-xl shadow-white/10 group-hover:scale-105 transition-transform" asChild>
                                    <Link href={`/practice/reading/test/${featuredMock.id}`}>
            ...
            <section className="mb-6">
                ...
                <div className="grid lg:grid-cols-3 gap-6">
                    {[...fullMockTests, ...practiceTests].map((test) => (
                        <Card key={test.id} className="group bg-white/70 dark:bg-slate-900/40 border-slate-200 dark:border-white/5 backdrop-blur-xl rounded-[1.5rem] overflow-hidden hover:bg-white dark:hover:bg-slate-800/60 transition-all duration-500 hover:-translate-y-2 shadow-sm hover:shadow-xl hover:shadow-indigo-500/5">
                            <CardHeader className="p-6 pb-4">
                                <div className="flex items-center justify-between mb-3">
                                    <Badge className={`${test.difficulty === 'Easy' ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : test.difficulty === 'Medium' ? 'bg-amber-500/10 text-amber-600 dark:text-amber-400' : 'bg-red-500/10 text-red-600 dark:text-red-400'} border-none rounded-full px-3 py-0.5 text-[10px] font-black uppercase tracking-widest`}>
                                        {test.difficulty}
                                    </Badge>
                                    <div className="flex items-center gap-2 text-slate-500">
                                        <Clock className="w-3.5 h-3.5" />
                                        <span className="text-[10px] font-bold uppercase">{test.durationMinutes} min</span>
                                    </div>
                                </div>
                                <CardTitle className="text-lg font-black text-[#020617] dark:text-white mb-1.5">{test.title}</CardTitle>
                            ...
                            <CardContent className="p-6 pt-0">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2 text-slate-500">
                                        ...
                                        <span className="text-[10px] font-bold uppercase">{test.questions.length} Questions</span>
                                    </div>
                                    <Button size="sm" className="h-9 px-4 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold group-hover:scale-105 transition-transform text-xs" asChild>
                                        <Link href={`/practice/reading/test/${test.id}`}>
            ...
            <section>
                <div className="grid lg:grid-cols-12 gap-8">
                    ...
                    <div className="lg:col-span-5">
                        <Card className="bg-indigo-600 dark:bg-indigo-600/20 border-none rounded-2xl overflow-hidden h-full">
